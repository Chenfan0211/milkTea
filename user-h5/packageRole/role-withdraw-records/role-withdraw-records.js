...
const DETAIL_URL = '/packageRole/role-withdraw-detail/role-withdraw-detail';
const CATEGORIES = [
  { id: 'all', label: '全部' },
  { id: 'pending', label: '审核中' },
  { id: 'processing', label: '处理中' },
  { id: 'success', label: '已到账' },
  { id: 'failed', label: '已驳回' }
];
...
// 记录时间形如 2026-09-21 10:05:22，取前 10 位得到 YYYY-MM-DD。
...
Page(
  withShare({
    ...
    syncRole() {
      ...
      if (!role) {
        this.setData({ ready: false, title: '提现记录' });
      ...
      this.setData({ ready: true, title: `${role.label}提现记录`, records }, () => this.applyFilter());
    },
    syncRecords() {
    ...
    // 单号搜索 + 时间范围同时生效；角标数量基于过滤后的集合，保证与列表所见一致。
    applyFilter() {
    ...
    confirmDate() {
      ...
      if (field === 'start') {
        ...
        // 开始时间晚于已选结束时间时，直接把结束时间抬到同一天，避免出现空区间。
