...
const [width, height, centerOffset, leftPadding, rightPadding, contentWidth, contentHeight] = probe.stdout.trim().split('|').map(Number)
...
assert.ok(Math.abs(leftPadding - rightPadding) <= 8, `商品主体左右留白必须基本一致，当前 ${leftPadding}/${rightPadding}px`)
...
assert.ok(/display:\s*flex/.test(mediaRule) && /align-items:\s*center/.test(mediaRule) && /justify-content:\s*center/.test(mediaRule), '适用商品图片容器必须水平垂直居中')
assert.ok(mediaRule.includes('height: 220rpx') && mediaRule.includes('background: var(--card-bg)'), '适用商品图片区必须使用 220rpx 白底画框')
assert.ok(/display:\s*block/.test(imageRule) && /width:\s*100%/.test(imageRule) && /height:\s*100%/.test(imageRule), '适用商品图片必须块级铺满并居中展示')
