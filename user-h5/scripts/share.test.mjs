...
const { DEFAULT_SHARE_IMAGE, buildShareAppMessage, buildShareTimeline, getShareTitle, withShare } = require(
  path.join(root, 'utils/share.js')
);
...
assert.equal(getShareTitle('pages/points-mall/points-mall'), '五零时光时光币商城', '时光币商城分享标题必须正确');
assert.equal(
  getShareTitle('pages/points-exchange/points-exchange'),
  '五零时光时光币兑换',
  '时光币兑换分享标题必须正确'
);
assert.equal(getShareTitle('pages/points-detail/points-detail'), '五零时光时光币明细', '时光币明细分享标题必须正确');
const exchangeShare = buildShareAppMessage('pages/points-exchange/points-exchange', {
  id: 'points-pet-food',
  from: 'test'
});
assert.equal(
  exchangeShare.path,
  '/pages/points-exchange/points-exchange?id=points-pet-food',
  '时光币兑换页必须保留 id 并过滤其他参数'
);
...
const publicTimeline = buildShareTimeline('pages/coupon-stores/coupon-stores', {
  couponId: 'coupon-1',
  token: 'secret'
});
assert.equal(publicTimeline.query, 'couponId=coupon-1', '公开页朋友圈分享必须只保留安全参数');

const productShare = buildShareAppMessage('pages/coupon-products/coupon-products', {
  couponId: 'coupon-001',
  storeId: 'store-001',
  token: 'secret'
});
assert.equal(
  productShare.path,
  '/pages/coupon-products/coupon-products?couponId=coupon-001&storeId=store-001',
  '适用商品页必须保留券号和门店号分享参数'
);
