...
// ---- 业务数据已落库：测试改从 DB seed 解析（scripts/lib/seed-data.mjs） ----
...
const pointsProducts = [...pointsProductSql.matchAll(POINTS_ROW_RE)]
  .map(m => ({
    id: m[2],
    name: m[3],
    image: m[4],
...
// 订单由后端生成，本地不再有 mock 可校验字段完整性；改为校验页面装配逻辑
assert.ok(
...
assert.equal(streakState.points, 27, '连续签到第7天必须获得每日1+额外20共21时光币');
assert.equal(streakState.continuousDays, 7, '连续签到天数必须累计到7');
const day8 = signInOnce(streakState, '2026-11-08', '2026-11-08 09:00').state;
assert.equal(day8.points, 28, '第8天只获得每日1时光币');
...
assert.ok(
  // 点单页门店态顶部内嵌真实 <map>（webview 渲染器），不再有「假地图」占位图；
  // 门店列表紧随地图下方，导航按钮仍走 wx.openLocation。
  menuWxml.includes('<map') &&
    menuWxml.includes('markers="{{pickerMarkers}}"') &&
...
// 商品列表金额单位：后端下发为「分」，菜单页必须在组装 productRows 时统一转换为「元」，
// 否则卡片会显示 ¥1600 而非 ¥16。转换由 normalizeSpecProduct 统一负责。
assert.ok(
  menuJs.includes('normalizeSpecProduct') && /normalizeSpecProduct\(\s*product\s*\)/.test(menuJs),
  '菜单商品必须在 buildProductRows 中经 normalizeSpecProduct 统一换算金额单位'
);
{
  ...
  assert.equal(normalized.specDetail.originalBasePrice, 16, '规格弹层基准价必须同为元，保持单一数据源');
}
assert.ok(
...
// 阶段 C 假数据清理：购物车不再预置商品，初始必须为空且不使用本地 mock 数据。
...
assert.ok(
  !/甜度|分甜|sweetness/.test(fs.readFileSync(path.join(root, 'data/mock.js'), 'utf8')),
  '商品与订单数据不得保留甜度规格'
);
