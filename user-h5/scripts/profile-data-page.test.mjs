...
globalThis.wx = {
  ...
  setStorageSync(key, value) {
    storage[key] = value;
    calls.push({ type: 'setStorage', key, value });
  },
...
assert.ok(
  wxss.includes('var(--brand-green)') &&
    ...
    wxss.includes('var(--radius-lg)') &&
    wxss.includes('var(--radius-pill)'),
  '个人资料页必须遵守设计 token'
);
const profileColorLiterals = [
  ...new Set(
    [...wxss.matchAll(/#[0-9A-Fa-f]{3,8}/g)].map(match => match[0]).filter(color => color.toUpperCase() !== '#FFFFFF')
  )
];
