...
const sourceFiles = walk(root).filter(file => !file.includes(`${path.sep}assets${path.sep}`) && !file.includes(`${path.sep}node_modules${path.sep}`))
...
for (const file of sourceFiles.filter(file => /\.(js|json|wxml|wxss)$/.test(file))) {
  ...
  if (runtimeFile && content.includes('/assets/temp/')) errors.push(`运行代码禁止引用 assets/temp: ${path.relative(root, file)}`)
...
if (!homeWxml.includes('join-card') || !homeWxml.includes('/assets/images/3x/join-banner.jpg') || homeWxml.includes('join-texture') || homeWxml.includes('join-card__shade') || homeWxml.includes('heroHeight') || homeWxml.includes('capsule-repair')) {
...
if (!/\.hero-wrap[\s\S]*height:\s*1040rpx/.test(homeWxss) || !/\.home-hero[\s\S]*width:\s*750rpx[\s\S]*height:\s*1040rpx/.test(homeWxss) || !/margin:\s*-13rpx\s+20rpx\s+0/.test(homeWxss) || !/\.home-card[\s\S]*border-radius:\s*24rpx/.test(homeWxss) || !/\.join-card[\s\S]*height:\s*200rpx[\s\S]*margin:\s*6rpx\s+20rpx\s+0/.test(homeWxss)) {
...
if (!profileWxml.startsWith('<scroll-view class="profile-scroll" scroll-y') || !/\.profile-scroll[\s\S]*height:\s*100vh/.test(profileWxss) || !profileWxml.includes('class="profile-hero"') || !profileWxml.includes('mode="aspectFill"') || !/\.profile-hero[\s\S]*width:\s*750rpx[\s\S]*height:\s*437rpx/.test(profileWxss) || !/\.profile-content[\s\S]*margin-top:\s*-127rpx[\s\S]*background:\s*transparent/.test(profileWxss)) {
...
if (!ordersWxml.includes('<navigation-bar') || !ordersWxml.includes('title="我的订单"')) errors.push('订单页必须使用自定义导航栏并显示我的订单标题')
if (!ordersWxml.includes('invoice-entry') || !ordersWxml.includes('/assets/icons/lucide/receipt.svg')) errors.push('订单页必须使用 Lucide receipt 图标作为开发票入口')
if (!ordersWxml.includes('orders-tabs') || !ordersWxml.includes('scroll-x')) errors.push('订单页必须包含可横向滚动的分类页签')
if (!ordersWxml.includes('order-card') || !ordersWxml.includes('order-product-single') || !ordersWxml.includes('order-product-grid')) errors.push('订单页必须同时支持单商品与多商品订单卡片')
if (!ordersWxml.includes('empty-state') || !ordersWxml.includes('filteredOrders.length')) errors.push('订单页必须为空分类结果提供空状态')
if (!ordersWxml.includes('tabbar-safe-space')) errors.push('订单页列表底部必须保留 TabBar 安全区')
if (!ordersWxss.includes('height: 100vh') || !ordersWxss.includes('min-height: 0')) errors.push('订单页必须使用视口高度的纵向 flex 骨架与自适应滚动区')
if (!/\.order-card__status\.is-canceled\s*\{/.test(ordersWxss)) errors.push('订单页必须区分已完成与已取消状态样式')
if (/#[0-9A-Fa-f]{3,8}\b|rgba?\(/.test(ordersWxss)) errors.push('订单页 WXSS 必须使用设计 token，禁止出现颜色字面量与 rgba()')
...
if (!cartBarWxmlSource.includes('cart-bar-dock') || !cartBarWxmlSource.includes('shopping-bag-white.svg') || !cartBarWxmlSource.includes('去结算')) {
...
if (tabPaths.length !== 5 || ![ '/pages/home/home', '/pages/menu/menu', '/pages/member/member', '/pages/orders/orders', '/pages/profile/profile' ].every((item, index) => item === tabPaths[index])) {
  errors.push('底部导航五个页面路径必须与 Tab 页面逐项一致')
}
if (!tabBarWxml.includes('open-type="switchTab"') || !tabBarWxml.includes('url="{{item.pagePath}}"') || tabBarWxml.includes('bindtap="switchTab"')) {
...
console.log(`项目结构校验通过: ${expectedPages.length} 个页面、${tabBarPages.length} 个 Tab，Lucide 图标与本地资源完整。`)
