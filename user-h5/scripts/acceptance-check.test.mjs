...
assert.ok(navigationBarWxml.includes('wx:if="{{search}}"') && navigationBarWxml.includes('search.svg'), '共享导航栏必须支持可选搜索按钮')
assert.ok(navigationBarWxss.includes('.weui-navigation-bar__search') && navigationBarWxss.includes('var(--line-color)'), '导航栏搜索按钮必须遵守设计系统样式')
...
assert.equal(stores.length, 3, '选择商品适用门店页必须保留三家现有门店')
...
assert.ok(pointsMallWxml.includes('search="{{true}}"') && pointsMallWxml.includes('bindsearch="handleSearch"'), '积分商城必须启用导航栏搜索入口')
...
assert.ok(/onShow\(\)\s*\{[\s\S]*?stores\.find\(store => store\.id === app\.globalData\.selectedStoreId\)[\s\S]*?updates\.currentStore/.test(menuJsSource), '点单页必须同步全局门店（onShow 按 selectedStoreId 更新 currentStore）')
...
assert.ok(/\.gift-agreement\s*\{[\s\S]*?align-items:\s*flex-start/.test(giftPurchaseWxss), '礼品卡协议区必须使用顶部对齐避免排版错位')
...
assert.equal(classicHerbal.products.length, 7, '店长推荐商品必须并入草本养生茶')
...
for (const product of classicHerbal.products) {
  assert.deepEqual(product.tags, ['年度热销', '五窨茉莉花茶'], '首屏商品标签必须与设计稿一致')
...
assert.ok((menuWxml.match(/show-scrollbar="\{\{false\}\}"/g) || []).length === 2, '分类栏和商品区必须隐藏滚动条')
...
assert.ok(specSheetJs.includes('selectOption') && specSheetJs.includes('toggleExpand') && specSheetJs.includes('increaseQuantity') && specSheetJs.includes('decreaseQuantity') && specSheetJs.includes("triggerEvent('addcart'") && specSheetJs.includes("triggerEvent('buy'"))
...
assert.ok(firstSpecProduct.specDetail && firstSpecProduct.specDetail.specGroups.length === 4, '商品必须补齐规格详情数据结构')
assert.equal(firstSpecProduct.specDetail.specGroups[0].options[0].label, '中杯', '默认份量必须为中杯')
assert.equal(firstSpecProduct.specDetail.specGroups[1].options[0].label, '7分甜', '默认甜度必须为 7 分甜')
assert.equal(firstSpecProduct.specDetail.specGroups[3].options[0].label, '不加马蹄粉圆', '默认加料必须为不加马蹄粉圆')
