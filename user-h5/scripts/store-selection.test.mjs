...
assert.ok(stores.every(store => store.cityCode && Number.isFinite(store.latitude) && Number.isFinite(store.longitude)), '每家门店必须有城市和经纬度')
assert.ok(stores.every(store => Number.isInteger(store.queueCount) && !Object.prototype.hasOwnProperty.call(store, 'queueText')), '每家门店必须有 queueCount 且不再保存 queueText')
assert.ok(stores.every(store => store.promotion === '新中式养生茶系列上新'), '所有门店必须使用统一的新中式养生茶促销文案')
...
assert.equal(findNearestStore(DEFAULT_CITY_CODE, { latitude: 28.2282, longitude: 112.9388 }).id, sortedStores[0].id, '最近门店必须与排序结果一致')
...
handleAppHide(4000)
assert.equal(resolveStorePreference(4000 + STORE_SELECTION_TTL).activeStoreId, 'store-004', '退出时间正好 30 分钟时必须保留门店')

handleAppHide(10000)
const expired = resolveStorePreference(10000 + STORE_SELECTION_TTL + 1)
assert.equal(expired.activeStoreId, null, '退出超过 30 分钟必须清除门店')
assert.equal(resolveStoreCatalog(10000 + STORE_SELECTION_TTL + 1).currentStore, null, '退出超时后必须停留选店状态，不得自动恢复最近门店')
