...
assert.deepEqual(page.data.favoriteStores.map(store => store.id), ['store-001'], '收藏页只能展示当前城市收藏门店')

page.handleRemoveFavorite({ currentTarget: { dataset: { id: 'store-001' } } })
...
toggleFavoriteStore('store-001')
page.loadFavorites()
page.handleSelectFavoriteStore({ currentTarget: { dataset: { id: 'store-001' } } })
