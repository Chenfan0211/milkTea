...
function buildSpecState(product = {}, options = {}) {
  ...
  for (const group of specGroups) {
    ...
    group.options = group.options.map(option => Object.assign({}, option, {
  ...
  const originalBasePrice = detail.originalBasePrice || startPrice
  const memberPrice = roundMoney(startPrice * discountRate + addOnTotal)
  const originalPrice = roundMoney(originalBasePrice + addOnTotal)
